...
class OwySpinner {
	...
	next() {
		...
		const str = ansiplace(`\u001b[${this.text.length + (frame.length * (this.options.dual ? 2 : 1)) + 2}D${this.color}${frame} {reset}${this.text}${this.options.dual ? ` ${this.color}${frame}` : ''}`);
	...
	/**
	 * @private Clears the spin interval
	 */
	_clear() {
